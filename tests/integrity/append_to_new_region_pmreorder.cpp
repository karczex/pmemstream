...
/* Pmreorder test for recovery after crash where user starts appending data
 * to new allocated regions */
...
void check_consistency(test_config_type test_config)
{
	...
	for (auto &region :
	     stream.helpers.allocate_regions(regions_count_to_allocate_on_check, TEST_DEFAULT_REGION_MULTI_SIZE)) {
...
int main(int argc, char *argv[])
{
	...
	test_config.stream_size = STREAM_METADATA_SIZE +
		2 * TEST_DEFAULT_BLOCK_SIZE * (regions_count_to_allocate_on_check + regions_count) *
			entries_in_region_count * 10;
